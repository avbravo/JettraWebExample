...
import io.jettra.wui.components.SessionTimeoutDialog;

public class SessionTimeoutDialogPage extends DashboardBasePage {

    public SessionTimeoutDialogPage() {
        super("SessionTimeoutDialog");
    }
    ...
    protected void initCenter(Center center, String username) {
        ...
        Header h1 = new Header(1, "SessionTimeoutDialog Showcase");
        ...
        codeBtn.setProperty("onclick", "document.getElementById('code-modal-std').style.display = 'block'");
        headerRow.add(codeBtn);
        
        container.add(headerRow);
        
        // --- Code Modal Dialog for Java Code ---
        io.jettra.wui.complex.Modal codeModal = new io.jettra.wui.complex.Modal("code-modal-std");
        ...
        codeTag.setProperty("id", "java-code-std");
        ...
        copyBtn.setProperty("onclick", "navigator.clipboard.writeText(document.getElementById('java-code-std').innerText).then(() => { this.innerText='Copied!'; setTimeout(() => this.innerText='Copy', 2000); })");
        
        Button closeBtn = new Button("Close");
        closeBtn.addClass("j-btn");
        closeBtn.setStyle("background", "transparent").setStyle("border-color", "var(--jettra-border)");
        closeBtn.setProperty("onclick", "document.getElementById('code-modal-std').style.display = 'none'");
        ...
        container.add(new Header(2, "SessionTimeoutDialog Example").setStyle("margin-top", "30px"));
        container.add(new Paragraph("Simulates session timeouts. Note: We do not instantiate it here as it would trigger a redirect on this page.").setStyle("margin-bottom", "20px"));
        
        Div alertBox = new Div();
        alertBox.setStyle("background", "rgba(255,165,0,0.2)").setStyle("padding", "15px").setStyle("border-left", "5px solid orange");
        alertBox.add(new Paragraph("The component runs Javascript on the page to manage session timeout and redirection automatically."));
