...
public class PersonaPage extends DashboardBasePage {
    ...
    public PersonaPage() { super("Gestión de Personas (Optimizado)"); }

    @Override
    protected void onInit(java.util.Map<String, String> params) {
        try { pageNumber = Integer.parseInt(params.getOrDefault("page", "1")); } catch (Exception e) {}
        super.onInit(params);
    }
    ...
    protected void initCenter(Center center, String username) {
        ...
        table.addHeaderRow(new Row(new TD(msg.getProperty("th.id", "ID")), new TD(msg.getProperty("th.name")), new TD(msg.getProperty("th.address")), new TD().add(addBtn)));

        List<PersonaModel> all = PersonaRepository.findAll();
        all.stream().skip((pageNumber-1)*10L).limit(10).forEach(p -> {
            Button editBtn = new Button("✏️").setId("btnEdit-" + p.getId()).addClickListener(() -> openModal("save", p));
            Button delBtn = new Button("🗑️").setId("btnDel-" + p.getId()).setStyle("color","red").addClickListener(() -> openModal("delete", p));
            table.addRow(new Row(new TD(p.getId()).setStyle("font-family","monospace").setStyle("font-size","11px"), new TD(p.getNombre()), new TD(p.getDireccion()), new TD().add(editBtn).add(delBtn)));
        });
    ...
    private void setupModal() {
        ...
        groupId.add(new Label("idV", msg.getProperty("lbl.id", "ID"))).add(inputIdVisible = new TextBox("text", "idV").setReadonly(true).setStyle("opacity","0.7"));
        form.add(groupId);

        form.add(groupNombre = new FormGroup()).add(new Label("n", msg.getProperty("lbl.name"))).add(inputNombre = new TextBox("text", "nombre"));
        form.add(groupDireccion = new FormGroup()).add(new Label("d", msg.getProperty("lbl.address"))).add(inputDireccion = new TextBox("text", "direccion"));
        
        deleteMsg = new Paragraph(msg.getProperty("msg.confirm.delete")).setStyle("color", "#f85149").setStyle("display", "none");
        form.add(deleteMsg);

        Div actions = new Div().setStyle("display","flex").setStyle("gap","10px").setStyle("margin-top","20px");
        actions.add(new Button(msg.getProperty("btn.cancel")).setType("button").setOnclick("document.getElementById('crudModal').style.display='none'; return false;"));
        actions.add(modalSubmitBtn = new Button(msg.getProperty("btn.save")).setType("submit").setBackgroundColor("#238636"));
    ...
    protected void onPost(java.util.Map<String, String> params) {
        ...
        if ("save".equals(action)) {
            ...
            JettraSyncManager.notifyChange("PersonaModel", (id==null||id.isEmpty())?SyncType.CREATE:SyncType.UPDATE, "user");
        } else if ("delete".equals(action)) {
            PersonaRepository.delete(id);
            JettraSyncManager.notifyChange("PersonaModel", SyncType.DELETE, "user");
        }
        try { redirect(currentExchange, JettraServer.resolvePath("/persona?page=" + pageNumber)); } catch (Exception e) {}
    }
}
